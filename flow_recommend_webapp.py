...
if st.button("유량 추천 실행"):
    ...
    for substance in substances:
        ...
        if low is None:
            ...
            before = round(random.uniform(low, high), 3)
            for _ in range(10):
                after = round(random.uniform(low, before), 3)
    ...
    st.write("### 결과")
    for substance, rep, before, after in results:
        st.write(f"- **입력 유해인자**: {substance}")
        st.write(f"  - 대표 유해인자: {rep}")
        st.write(f"  - 추천 유량 (측정 전): {before}")
        st.write(f"  - 추천 유량 (측정 후): {after}")
        st.markdown("---")
